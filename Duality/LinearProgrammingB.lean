...
@[coe] def coeNN [Field R] [LinearOrder R] [IsStrictOrderedRing R] : (I → R≥0) → (I → R) := (Subtype.val ∘ ·)

instance [Field R] [LinearOrder R] [IsStrictOrderedRing R] : Coe (I → R≥0) (I → R) := ⟨coeNN⟩
...
def StandardLP.IsSolution [Field R] [LinearOrder R] [IsStrictOrderedRing R] (P : StandardLP I J R) (x : J → R≥0) : Prop :=
  P.A *ᵥ coeNN x ≤ P.b
...
def StandardLP.Reaches [Field R] [LinearOrder R] [IsStrictOrderedRing R] (P : StandardLP I J R) (r : R) : Prop :=
  ∃ x : J → R≥0, P.IsSolution x ∧ P.c ⬝ᵥ coeNN x = r

/-- Linear program `P` is feasible iff there exists a solution to `P`. -/
def StandardLP.IsFeasible [Field R] [LinearOrder R] [IsStrictOrderedRing R] (P : StandardLP I J R) : Prop :=
  ∃ r : R, P.Reaches r

/-- Linear program `P` is bounded by `r` iff every value reached by `P` is
    greater or equal to `r` (i.e., `P` is bounded by `r` from below). -/
def StandardLP.IsBoundedBy [Field R] [LinearOrder R] [IsStrictOrderedRing R] (P : StandardLP I J R) (r : R) : Prop :=
  ∀ p : R, P.Reaches p → r ≤ p

/-- Linear program `P` is unbounded iff values reached by `P` have no lower bound. -/
def StandardLP.IsUnbounded [Field R] [LinearOrder R] [IsStrictOrderedRing R] (P : StandardLP I J R) : Prop :=
  ¬∃ r : R, P.IsBoundedBy r
...
theorem StandardLP.weakDuality [Fintype I] [Field R] [LinearOrder R] [IsStrictOrderedRing R] {P : StandardLP I J R}
...
private lemma StandardLP.toE_dotProduct_apply (P : StandardLP I J R) (x : J → R≥0) :
    toE (P.c ⬝ᵥ coeNN x) = (toE ∘ P.c ᵥ⬝ x) := by
...
private lemma StandardLP.toE_mulVec_apply (P : StandardLP I J R) (x : J → R≥0) (i : I) :
    toE ((P.A *ᵥ coeNN x) i) = (P.A.map toE ₘ* x) i := by
...
private lemma StandardLP.toValidELP.isSolution_iff (P : StandardLP I J R) (x : J → R≥0) :
  ...
  show P.A.map toE ₘ* x ≤ toE ∘ P.b ↔ P.A *ᵥ coeNN x ≤ P.b
...
private lemma StandardLP.toValidELP_isUnbounded_iff (P : StandardLP I J R) :
  ...
  constructor <;> intro hP hr <;> apply hP <;> simpa only [P.toValidELP_isBoundedBy_iff] using hr
...
theorem StandardLP.optimum_neq_none (P : StandardLP I J R) :
    P.optimum ≠ none :=
  P.toValidELP_optimum_eq ▸ P.toValidELP.optimum_neq_none
