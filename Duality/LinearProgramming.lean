...
| some p, some q => p = -q  -- opposite values; includes `⊥ = -⊤` and `⊤ = -⊥`
...
lemma EF.smul_lt_smul_left {k : F≥0} (hk : 0 < k) (r s : F∞) :
  ...
  | (q : F) =>
    ...
    | (p : F) =>
      ...
      rw [EF.coe_lt_coe_iff]
      exact ⟨fun h => by
        have hk_ne : (↑k : F) ≠ 0 := ne_of_gt hk
        have h_inv : 0 < (↑k : F)⁻¹ := inv_pos.mpr hk
        have h2 := mul_lt_mul_of_pos_left h h_inv
        rw [←mul_assoc, ←mul_assoc] at h2
        have h_cancel : (↑k : F)⁻¹ * ↑k = 1 := inv_mul_cancel₀ hk_ne
        rw [h_cancel, one_mul, one_mul] at h2
        exact h2
      , fun h => mul_lt_mul_of_pos_left h hk⟩

lemma EF.smul_le_smul_left {k : F≥0} (hk : 0 < k) (r s : F∞) :
    k • r ≤ k • s ↔ r ≤ s := by
  have h := EF.smul_lt_smul_left hk s r
  exact ⟨fun hle => by contrapose hle; push Not at hle ⊢; exact h.mpr hle,
         fun hle => by contrapose hle; push Not at hle ⊢; exact h.mp hle⟩
...
lemma oppositesOpt_comm (p q : Option F∞) : OppositesOpt p q ↔ OppositesOpt q p := by
  cases p <;> cases q <;> simp [OppositesOpt]
  exact ⟨fun h => by rw [h, neg_neg], fun h => by rw [h, neg_neg]⟩
